...
> 本文转载至：https://yeasy.gitbook.io/docker_practice/image/commit
...
我们以交互式终端方式进入 `webserver` 容器，并执行了 `bash` 命令，也就是获得一个可操作的 Shell。然后，我们用 `<h1>Hello, Docker!</h1>` 覆盖了 `/usr/share/nginx/html/index.html` 的内容。
...
我们可以在 `docker image` 中看到这个新定制的镜像：
