...
通过Pod的名称我们就可以发现，在Deployment控制器管理的Pod命名有如下特点：

![](../images/26.png)

**红色部分是Deployment控制器的名称，绿色部分是ReplicaSet（RS）的名称，蓝色部分是Pod的名称**。以`pc-deployment-5d89bdfbf9-65qcw`Pod为例，通过该名称我们能快速确定管理该Pod的Deployment的名称为`pc-deployment`，管理这个Pod的ReplicaSet的名称是`pc-deployment-5d89bdfbf9`。

在上述重建过程中，重建过程前后管理Pod的ReplicaSet发生了变化，实际上老的RS并没有删除，保留它是为了方便做版本回滚。
...
# 可以发现有两个版本记录，说明完成过一次升级
```

需要注意的是有些同学可能会发现`CHANGE-CAUSE`这一列是空的：

![](../images/37.png)

这是我们在创建Deployment时会指定`--record`参数，该参数用于记录kubectl命令对Deployment的变更命令：

```shell
kubectl create -f pc-deployement.yaml --record
```
...
# 一旦想回滚到哪个版本，只需要将当前版本RS的pod期望数量降为0，然后将回滚版本RS的pod期望数量提升为目标数就可以了
[root@k8s-master01 ~]# kubectl get rs -n dev
NAME                       DESIRED   CURRENT   READY   AGE
pc-deployment-6696798b78   4         4         4       78m
pc-deployment-966bf7f44    0         0         0       37m
pc-deployment-c848d767     0         0         0       71
```

版本回滚后，我们再去查看Deployment升级历史，会发现原先的1号版本消失，新建了一个3号版本，而3号版本使用的RS就是原先1号版本创建的RS：
...
上述image是由笔者备份到自己的DockerHub上，如果你的网络不好，也可以使用阿里云镜像：
...
创建完成后，我们查看`kube-system`命名空间下的Pod是否启动成功：
