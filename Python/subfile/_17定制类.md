...
`__str__` 方法是一个特殊的方法，它在用 print 函数或 str() 函数转换对象为字符串时被调用。类似于Java中的 `toString` 方法，可以改变对象默认输出行为。
...
if __name__ == '__main__':
    stu = Student("张三")
    print(stu) # Output：<__main__.Student object at 0x000001B55E940790>
```

打印出一堆`<__main__.Student object at 0x000001B55E940790>`，不好看。
...
if __name__ == '__main__':
    stu = Student("张三")
    print(stu) # Output：Student:(name:张三)
```
