...
例如，A 连上 B 之后，发送了 1、2、3 三个包，但是发送 3 的时候，中间丢了，或者绕路了，于是重新发送。重新发送3号包后 A 掉线了，又重新连上 B 后，序号又从 1 开始，然后发送 2，但是压根没想发送 3，但是上次绕路的那个 3 又回来了，发给了 B，B 自然认为，这就是下一个包，于是发生了错误。
...
A 开始说“不玩了”，B 说“知道了”，这个回合，是没什么问题的，因为在此之前，双方还处于合作的状态，如果 A 说“不玩了”，没有收到回复，则 A 会重新发送“不玩了”。但是这个回合结束之后，就有可能出现异常情况了，因为已经有一方撕破脸。
...
- **由于TCP协议是一个可靠协议（努力达到可靠），所以TCP协议中每次发送的包，都期望有一个应答包，来应证该包已经被对方收到**。
- TCP在建立连接时三次握手：A->B发送完毕后，A并不知道B接收到了包，所以B发送一个应答包给A（B->A），但是此次应答B没法证实A已经接收到了这个应答，所以A需要再发送一个应答之应答包（A->B），B接收到这个包后基本上可以确定，A和B之间的通道在两个方向上都是畅通的。
- TCP断开连接的四次挥手：A->B：我要断开连接了，B->A：我知道你要断开了（此时处于半连接状态，A->B的连接断开了，但是B->A的还未断开），B->A：我也要断开连接了，A->B：好的，拜拜。