...
public interface Lock {
    ...
    //仅在调用时锁为空闲状态才获取该锁。如果可用，则获取锁定，并立即返回值为true 。 如果锁不可用，则此方法将立即返回值为false 。 
    boolean tryLock();
...
### 3.1 `lock`、`tryLock`和`lockInterruptibly`

- `tryLock`用于尝试获取锁，如果获取到锁返回true，如果未获取到锁则返回false。这个方法不管是否获取到锁都会立即返回。
- `lock`用于获取锁，当前线程会进入等待队列中等待锁的释放，这个等待过程是阻塞的，并且不会响应线程的中断请求。
- `lockInterruptibly`在等待锁的途中可以响应线程中断。显示锁的`lock`方法和内置锁获取锁的时候都会发生阻塞（等待其他线程释放锁），但是如果在等待锁的队列中，它们都不会响应中断，直到获取到锁后，拿到时间片才开始响应中断。而使用`lockInterruptibly`方法在锁的等待队列中也可以随时响应中断。
