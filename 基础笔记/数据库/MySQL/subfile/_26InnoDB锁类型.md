...
读取操作分为快照读（乐观锁， MVCC机制实现），和当前读（X、S锁）：

- 快照读：使用MVCC机制实现，常规的select的语句都属于快照读，由于MVCC中 ReadView 的存在，当前事务通常无法读取当前事务开始后其他事务commit的数据，也就避免了绝大多数幻读的出现。

  > MVCC无法完全避免快照读，参考：[《什么是MVCC机制》](./_10什么是MVCC机制.md)

- 当前读：当前读是指读取数据库中最新的数据，读取的数据会加锁，以保证数据的一致性和安全性。也就是我们前面所提到的行级 X、S锁，`select ... for update` 和 `select ... lock in share mode` 就是典型的当前读。
...
Gap locks的目的就是为了**防止其他事务往索引的间隙插入数据**，以此来避免出现幻读。在Mysql的`REPEATABLE-READ`隔离级别下，Gap locks默认启用。禁用方式很简单，把隔离级别设置为`READ_COMMITTED`即可。
...
上面的sql会使用Gap lock，(20,26)之间的间隙会被锁定，其他事务无法往这个区间插入数据。
