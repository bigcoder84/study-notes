# 助力SpringBoot自动配置的条件注解原理揭秘

> 本文参考至：[助力SpringBoot自动配置的条件注解原理揭秘 SpringBoot源码（三） - 掘金 (juejin.cn)](https://juejin.cn/post/6844904090477166599)

因为SpringBoot自动配置原理是基于其大量的条件注解`ConditionalOnXXX`，因此，本节我们先来撸下Spring的条件注解的相关源码。

## 一. SpringBoot的派生条件注解

我们都知道，SpringBoot自动配置是需要满足相应的条件才会自动配置，因此SpringBoot的自动配置大量应用了条件注解`ConditionalOnXXX`。
...
其实上述条件注解都是派生注解，我们以`ConditionalOnClass`为例：
...
举个栗子，我们来看下如何使用`@ConditionalOnClass`和`@ConditionalOnProperty`这两个注解。

定义 `StudentBeanProperties` 用于封装配置文件配置信息：

```java
@ConfigurationProperties(prefix = "condition.test")
public class StudentBeanProperties {
    private Integer id;
    private String name;
...
@EnableConfigurationProperties(StudentBeanProperties.class)
@ConditionalOnProperty(prefix = "condition.test", matchIfMissing = false, value = {"name", "id"})
public class MyStudentBeanConfiguration {

    @Autowired
    private StudentBeanProperties studentBeanProperties;

    @Bean
    public StudentBean studentBean() {
        StudentBean studentBean = new StudentBean();
        studentBean.setId(studentBeanProperties.getId());
        studentBean.setName(studentBeanProperties.getName());
        System.out.println("创建StudentBean,id:" + studentBean.getId() + ";name:" + studentBean.getName());
        return studentBean;
    }
}
...
`MyStudentBeanConfiguration`这个自动配置类应用了`@ConditionalOnClass`和`ConditionalOnProperty`两个条件注解，那么只有在满足:`classpath`中存在`StudentBean.class`和配置了`condition.test.name`和`condition.test.age`属性**这两个条件**的情况下才会创建`StudentBean`这个`bean`。

在 `application.yml` 配置 `condition.test.name` 和 `condition.test.id` 两个属性：
...
如果我们此时删除ID属性的配置，重新启动SpringBoot，则会发现并不会加载StudentBean。
...
`ConditionEvaluator`的`shouldSkip`方法中调用了`LinuxCondition`的`matches`方法，自然我们再去看看`ConditionEvaluator`的`shouldSkip`的方法执行了什么逻辑。
...
内置的`Condition`接口的具体实现类虽然有多个，但只有`ProfileCondition`不是测试相关的，因此可以说真正的内置的`Condition`接口的具体实现类只有`ProfileCondition`一个，非常非常少,这跟SpringBoot的大量派生条件注解形成了鲜明的对比。`ProfileCondition`大家都知道，是跟环境有关，比如我们平时一般有`dev`,`test`和`prod`环境，而`ProfileCondition`就是判断我们项目配置了哪个环境的。下面是`ProfileCondition`的源码，很简单，这里就不分析了。
...
## 五. 如何扩展SpringBootCondition

前文我们知道了如何扩展Spring的`Condition`接口，那么我们该如何扩展SpringBoot的`SpringBootCondition`类呢？

推荐阅读[springboot之使用SpringBootCondition](https://link.juejin.cn/?target=https%3A%2F%2Fblog.csdn.net%2Fzhanglu1236789%2Farticle%2Fdetails%2F78999496)获得答案
